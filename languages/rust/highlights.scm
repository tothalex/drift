; drift's curated rust highlight stack: the grammar's bundled query
; (tree-sitter-rust v0.24.2) followed by drift's supplements, which
; close the visible gaps against nvim-treesitter's richer queries:
; plain identifiers are variables, path::segments take the type color,
; call-argument parens get their own token, lifetimes and enum-variant
; constructors match editor coloring. Same-range collisions resolve by
; drift's token precedence, so the bundled query's more specific
; captures (functions, types, macros) still win.
; Identifiers
